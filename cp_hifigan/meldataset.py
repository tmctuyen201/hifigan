...
def load_wav(full_path):
    audio, sampling_rate = librosa.load(full_path, sr=None)
...
def mel_spectrogram(y, n_fft, num_mels, sampling_rate, hop_size, win_size, fmin, fmax, center=False):
    ...
    if fmax not in mel_basis:
        mel = librosa_mel_fn(sr=sampling_rate,           # Sample rate của tín hiệu âm thanh
                             n_fft=n_fft,     # Kích thước cửa sổ FFT
                             n_mels=num_mels,   # Số Mel bins
                             fmin=fmin,       # Tần số thấp nhất
                             fmax=fmax)
